const sortBtns = q("input[type=radio]", true);
...
function handleChange (event) {
    if(event.target.checked){
        sendMessageToContentScript({type: "sort", sort: event.target.value})
    }
...
async function sendMessageToContentScript(message) {
    ...
    try {
        ...
        if (error) {
            const errContainer = q(".error");
            errContainer.innerText = "Sorry, page not fully loaded";
            setTimeout(() => {
